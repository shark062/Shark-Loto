...
import { providers, evolutionLog, recalcPriorities, getEffectiveApiKey } from "./aiProviders";
...
async function callProvider(provider: ProviderConfig, prompt: string): Promise<{ text: string; latencyMs: number }> {
  ...
  if (!apiKey) throw new Error(`Variável de ambiente não configurada para o provider ${provider.type}`);
